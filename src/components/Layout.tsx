...
import { X, Home, Users, Clock, FileText, BarChart3, User, ChevronLeft, ChevronRight, Moon, Sun, Power, Archive, Scale, MessageCircle, CreditCard, Receipt } from 'lucide-react';
...
const menuItems = [
  ...
  { id: 'expenses', label: 'Dépenses', icon: Receipt, color: 'rose' },
...
export default function Layout({ children, currentPage, onPageChange }: LayoutProps) {
  ...
  const getColorClasses = (color: string) => {
    const colorMap = {
      ...
      rose: {
        active: 'bg-gradient-to-r from-rose-500 to-rose-600',
        icon: 'bg-rose-100 dark:bg-rose-900/30',
        iconHover: 'group-hover:bg-rose-100 dark:group-hover:bg-rose-900/30',
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-gray-50 dark:bg-slate-950 lg:overflow-hidden">
      ...
      {/* Main content */}
      <div className={`pt-[64px] transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-72'} flex flex-col lg:h-full lg:overflow-hidden`}>
        {/* Page content */}
        <main className="flex-1 lg:min-h-0 lg:overflow-y-auto p-4 md:p-6 lg:p-8 pb-4 md:pb-8 lg:pb-12">
          <div key={currentPage} className="animate-in fade-in slide-in-from-bottom-2 duration-300 ease-out">
            {children}
          </div>
        </main>
      </div>
